function out1 = newcf(varargin)
%NEWCF Create a cascade-forward backpropagation network.
%
%  Obsoleted in R2010b NNET 7.0.  Last used in R2010a NNET 6.0.4.
%  The recommended function is <a href="matlab:doc cascadeforwardnet">cascadeforwardnet</a>.
%
%  Syntax
%
...
%      net = newcf(P,T,5);
%
%    Here the network is evaluated and its output plotted against
%    the targets.
%
%      Y = net(P);
%       plot(P,T,P,Y,'o')
...
%     output is plotted.
%
%      net.<a href="matlab:doc nnproperty.net_trainParam">trainParam</a>.<a href="matlab:doc nnparam.epochs">epochs</a> = 50;
%      net = train(net,P,T);
%      Y = net(P);
%      plot(P,T,P,Y,'o')
%
...
if (~isa(pr,'double')) | ~isreal(pr) | (size(pr,2) ~= 2)
  error(message('nnet:NNet:InputRanges'))
end
if any(pr(:,1) > pr(:,2))
  error(message('nnet:NNet:RangesOutOfOrder'))
end
if isa(s,'cell')
  if (size(s,1) ~= 1)
    error(message('nnet:NNet:LayerSizes'))
